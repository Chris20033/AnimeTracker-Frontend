...
export function StatisticsCards({ totalAnime, completedAnime, totalEpisodesWatched, averageScore, variant = 'default' }: StatisticsCardsProps) {
  const cards = [
    { label: 'Anime total', value: formatStatNumber(totalAnime), caption: 'Entradas en biblioteca' },
    { label: 'Completados', value: formatStatNumber(completedAnime), caption: 'Historias terminadas' },
    { label: 'Episodios vistos', value: formatStatNumber(totalEpisodesWatched), caption: 'Tiempo invertido' },
    { label: 'Score promedio', value: formatAverageScore(averageScore), caption: 'Calificacion personal' },
  ]
  ...
  return (
    <section className={gridClassName} aria-label="Resumen de estadisticas">
      {cards.map((card) => (
