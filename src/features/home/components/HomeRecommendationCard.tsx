...
export function HomeRecommendationCard({ recommendation }: HomeRecommendationCardProps) {
  return (
    <article className="grid overflow-hidden rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--surface)] shadow-[0_18px_48px_var(--shadow)]">
      ...
      <div className="p-4">
        <h3 className="line-clamp-2 text-base font-black leading-6 text-[var(--page-fg)]">{recommendation.title}</h3>
        <div className="mt-3 flex items-center justify-between gap-3 text-sm font-bold text-[var(--muted)]">
