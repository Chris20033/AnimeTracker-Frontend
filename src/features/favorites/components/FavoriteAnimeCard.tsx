    ...
    <Link to={`/anime/${anime.source}/${anime.externalId}`} className="group ledger-inset grid grid-cols-[3.5rem_minmax(0,1fr)_auto] items-center gap-3 p-2.5 outline-none transition hover:-translate-y-0.5 hover:bg-[var(--surface)] focus:ring-4 focus:ring-[var(--focus)]">
      <div className="overflow-hidden rounded-[0.8rem] bg-[var(--surface-inset)]">
        <div className="aspect-[3/4]">
          {anime.imageUrl ? <img src={anime.imageUrl} alt={anime.title} className="size-full object-cover transition duration-300 group-hover:scale-105" /> : <div className="grid size-full place-items-center text-xs font-black text-[var(--soft)]">No image</div>}
        </div>
      </div>
      ...
      <span className="grid size-8 place-items-center rounded-full border border-amber-300/60 bg-amber-400/15" title="Favorite" aria-label="Favorite">
