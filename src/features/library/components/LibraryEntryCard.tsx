...
export function LibraryEntryCard({ entry }: LibraryEntryCardProps) {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setErrors({ form: getLibraryErrorMessage(error, 'No se pudo actualizar esta entrada.') })
    }
  }

  async function handleDelete() {
    const confirmed = window.confirm(`Eliminar ${entry.anime.title} de tu biblioteca?`)
    ...
    try {
      ...
      setErrors({ form: getLibraryErrorMessage(error, 'No se pudo eliminar esta entrada.') })
    }
  }
    ...
    <article className={`ledger-panel overflow-hidden border-l-4 p-3 sm:p-4 ${statusTone.accentClassName}`}>
      ...
      <details className="mt-3 rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--surface-inset)]">
        ...
        <form onSubmit={handleSubmit} className="grid gap-4 border-t border-[var(--line)] p-4">
          ...
          <label className="grid gap-2 text-sm font-black text-[var(--page-fg)]">
            Notas
            <textarea rows={3} maxLength={500} value={formState.notes} onChange={(event) => updateField('notes', event.target.value)} placeholder="Reacciones, arcos pendientes o por que lo pausaste..." className="resize-none rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--surface-inset)] px-3 py-2 font-semibold text-[var(--page-fg)] outline-none focus:ring-4 focus:ring-[var(--focus)]" />
...
function getLibraryErrorMessage(error: unknown, fallback: string) {
  if (isAxiosError(error)) {
    ...
    if (code === 'VALIDATION_ERROR') {
      return 'Revisa episodios, score y notas. El score debe ser del 1 al 10 o Sin score.'
    }

    if (code === 'RESOURCE_NOT_FOUND') {
      return 'No encontramos esta entrada en tu biblioteca. Recarga la pagina.'
    }
