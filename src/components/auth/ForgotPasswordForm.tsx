...
import { validateForgotPasswordForm, type ForgotPasswordFormErrors } from '@/utils/validate-forgot-password-form'
...
export function ForgotPasswordForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setSuccessMessage('Si el correo existe, te enviaremos un enlace para restablecer tu contrasena.')
    } catch (error) {
      setErrors({ form: isAxiosError(error) ? 'Revisa el correo ingresado e intenta nuevamente.' : 'No se pudo solicitar la recuperacion.' })
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="ledger-panel p-5 sm:p-7">
      <div className="mb-6 border-b border-[var(--line)] pb-5">
        ...
        <h2 className="mt-2 text-2xl ledger-title">Solicitar enlace temporal</h2>
      </div>
      ...
      <div className="grid gap-5">
        ...
        <SubmitButton isLoading={forgotPasswordMutation.isPending} loadingLabel="Enviando enlace...">
          Enviar enlace
        </SubmitButton>
