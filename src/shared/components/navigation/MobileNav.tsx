  ...
  return (
    <div className="sm:hidden">
      <div className="flex items-center gap-2">
        <ThemeToggle />
        <button type="button" aria-label={isOpen ? 'Close main menu' : 'Open main menu'} aria-expanded={isOpen} aria-controls="mobile-main-nav" onClick={() => setIsOpen((current) => !current)} className="grid min-h-10 min-w-10 place-items-center rounded-full border border-[var(--line)] bg-[var(--surface)] text-[var(--page-fg)] shadow-[0_14px_32px_var(--shadow)] outline-none transition hover:bg-[var(--surface-strong)] focus:ring-4 focus:ring-[var(--focus)]">
          <img src={iconUrl} alt="" className="size-5" />
        </button>
      </div>

      {isOpen ? (
        <nav id="mobile-main-nav" aria-label="Mobile main navigation" className="absolute inset-x-3 top-[calc(100%-0.35rem)] z-30 rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--surface-strong)] p-2 shadow-[0_24px_70px_var(--shadow-strong)]">
          {session ? <p className="px-3 pb-2 pt-1 text-xs font-semibold text-[var(--muted)]">Active session: <span className="font-black text-[var(--accent-strong)]">{session.user.username}</span></p> : null}
          <div className="grid gap-1">
            <NavLink to="/" onClick={closeMenu} className={({ isActive }) => mobileLinkClass(isActive)}>Home</NavLink>
            <NavLink to="/anime" onClick={closeMenu} className={({ isActive }) => mobileLinkClass(isActive)}>Anime</NavLink>
            {isAuthenticated ? (
              <>
                <NavLink to="/library" onClick={closeMenu} className={({ isActive }) => mobileLinkClass(isActive)}>Library</NavLink>
                <NavLink to="/dashboard" onClick={closeMenu} className={({ isActive }) => mobileLinkClass(isActive)}>Dashboard</NavLink>
                <NavLink to="/profile" onClick={closeMenu} className={({ isActive }) => mobileLinkClass(isActive)}>Profile</NavLink>
              ...
              <>
                ...
                <NavLink to="/registro" onClick={closeMenu} className={({ isActive }) => mobileLinkClass(isActive)}>Sign up</NavLink>
              </>
            )}
          </div>
        </nav>
